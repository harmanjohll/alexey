...
/* ── Theme ── */
function initTheme() {
  var saved = localStorage.getItem('alexey_theme');
...
function toggleTheme() {
  ...
  localStorage.setItem('alexey_theme', isDark ? 'light' : 'dark');
}
...
function renderLogbook() {
  ...
  for (var i = logEntries.length - 1; i >= 0; i--) {
    ...
    html += '<span class="log-entry-ts">' + e.ts + '</span>';
...
function addLogEntry() {
  ...
  logEntries.push({type: type, text: text, ts: new Date().toLocaleString(), ctx: ctx});
  document.getElementById('logText').value = '';
  renderLogbook();
  localStorage.setItem('alexey_fractal_logbook', JSON.stringify(logEntries));
}
  ...
  try {
    var r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1000,
        system: 'You are COACH \u2014 a Socratic science mentor for Alexey. He has been exploring fractal growth via Diffusion-Limited Aggregation (DLA) and Koch snowflake geometry. He can control sticking probability, seed type, iteration depth, and ruler size, and measures fractal dimension via M(r) regression and log-log ruler analysis. Your rules: Ask EXACTLY ONE question per response. Never give Alexey the answer. Never explain the science to him \u2014 make him work it out. Reference his specific data and his own words back to him. Accessible language for a bright, curious 14-year-old. No effusive praise. No filler. Just one sharp question. End every response with a question mark.',
        messages: convHist
      })
    });
    var d = await r.json();
    ...
    document.getElementById('coachReplySec').style.display = 'block';
  } catch (e) {
...
async function sendCoachReply() {
  ...
  thr.innerHTML += '<div class="tcoach"><div class="tcoach-lbl">coach\'s question</div>' + prevQ + '</div>';
  thr.innerHTML += '<div class="talex"><div class="talex-lbl">alexey</div>' + reply + '</div>';
  ...
  try {
    var r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 1000,
        system: 'You are COACH in a continuing Socratic dialogue with Alexey, 14, about his fractal experiments. Ask exactly one follow-up question. No preamble. No praise. Reference his answer. End with a question mark.',
        messages: convHist
      })
    });
    var d = await r.json();
...
(function init() {
  ...
  // Restore logbook
  try {
    var saved = localStorage.getItem('alexey_fractal_logbook');
    if (saved) { logEntries = JSON.parse(saved); renderLogbook(); }
  } catch (e) {}
})();
