...
// ── Theme, lock, coach, export: delegated to ../_common/ ──
...
var COACH_PROMPT_FOLLOWUP = 'You are COACH in a continuing Socratic dialogue with Alexey, 14, about his football xG experiment. Ask exactly one follow-up question. No preamble. No praise. Reference his answer. End with a question mark.';
...
// ── Hypothesis lock: delegated to Lock.attach (see DOMContentLoaded) ──
function onHypothesisLocked() {
  locked = true;
  var hint = document.getElementById('labHint');
  if (hint) hint.textContent = 'Click on the pitch to place shots';
  redrawMain();
}
...
function runMC() {
  ...
  Store.set(EXP_ID, 'results', {
    ...
    p0: result.p0, p2: result.p2plus, shots: shots.length, model: currentModel
  });
}
...
// ── AI Coach (delegates to Coach.ask via CF Worker proxy) ──
...
document.addEventListener('DOMContentLoaded', function () {
  // Hypothesis lock (auto-restores from Store if present)
  Lock.attach(EXP_ID, {
    textareaId: 'hypText',
    buttonId: 'lockBtn',
    timestampId: 'hypTS',
  ...
  if (convHist.length > 0) {
    ...
    var reply = document.getElementById('replySec');
    if (reply) reply.style.display = 'block';
  }

  // Restore interpretation
  var savedInterp = Store.get(EXP_ID, 'interpretation');
  if (savedInterp) document.getElementById('interpText').value = savedInterp;
  ...
  if (window.Exporter) {
    Exporter.mountBanner('exportMount', {
      ...
      description: 'Expected Goals (xG) — geometric and logistic models, Monte Carlo distributions, and real league comparisons.',
      figureCanvasId: 'mcChart'
    });
  }
});
